  ...
  handler: async (ctx, args) => {
    ...
    for (const row of args.rows) {
      ...
      const fields = {
        ...
        hostNames: row.hostNames?.split(";").map((item) => item.trim()).filter(Boolean) ?? [],
        topics: row.topics?.split(";").map((item) => item.trim()).filter(Boolean) ?? [],
        wantsToMeet: row.wantsToMeet?.split(";").map((item) => item.trim()).filter(Boolean) ?? [],
